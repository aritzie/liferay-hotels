...
public class RoomManagementToolbarDisplayContext extends BaseManagementToolbarDisplayContext {

	public RoomManagementToolbarDisplayContext(HttpServletRequest httpServletRequest,
		...
		return new CreationMenu() {
			{
				addDropdownItem(dropdownItem -> {
					dropdownItem.setHref(liferayPortletResponse.createRenderURL(),
							...
							MVCCommandsNames.EDIT_ROOM,
							"redirect",
							currentURLObj.toString(),
							"hotelId",
							ParamUtil.getLong(request, "hotelId"));						
					dropdownItem.setLabel("add-room");
				});
			}
		};
	}
	...
	protected String getOrderByCol() {
		return ParamUtil.getString(request, "orderByCol", "number");
	}
	...
	public String getSearchActionURL() {
		...
		searchUrl.setProperty("mvcRenderCommandName", MVCCommandsNames.VIEW_HOTEL);
	...
	public List<ViewTypeItem> getViewTypeItems() {
		...
		portletURL.setParameter("mvcRenderCommandName", MVCCommandsNames.VIEW_HOTEL);
		int delta = ParamUtil.getInteger(request, SearchContainer.DEFAULT_DELTA_PARAM);
		if(delta > 0) {
			portletURL.setParameter("delta", String.valueOf(delta));
		}
		String orderByCol = ParamUtil.getString(request, "orderByCol", "number");
	...
	protected List<DropdownItem> getOrderByDropdownItems() {
		return new DropdownItemList() {
			{
				add(dropdownItem -> {
					dropdownItem.setActive("number".equals(getOrderByCol()));
					dropdownItem.setHref(_getCurrentSortingURL());
					dropdownItem.setLabel(LanguageUtil.get(request, "number"));
				});
			}
		};
	}
