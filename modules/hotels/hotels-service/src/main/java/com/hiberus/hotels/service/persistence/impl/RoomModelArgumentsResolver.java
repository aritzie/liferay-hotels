...
/**
 * The arguments resolver class for retrieving value from Room.
 *
 * @author Brian Wing Shun Chan
 * @generated
 */
@Component(service = ArgumentsResolver.class)
public class RoomModelArgumentsResolver implements ArgumentsResolver {
	...
	public String getClassName() {
		return RoomImpl.class.getName();
	}

	@Override
	public String getTableName() {
		return RoomTable.INSTANCE.getTableName();
	}

	private static Object[] _getValue(
		RoomModelImpl roomModelImpl, String[] columnNames, boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				arguments[i] = roomModelImpl.getColumnOriginalValue(columnName);
			}
			else {
	...
	static {
		...
		orderByColumnsBitmask |= RoomModelImpl.getColumnBitmask("hotelId");
		orderByColumnsBitmask |= RoomModelImpl.getColumnBitmask("number_");
